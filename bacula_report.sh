...
for lines in `cat $TMPFILE`; do
  SUM=0
  RES=`$COMMAND "USE bacula; SELECT Job,JobBytes,JobStatus,PurgedFiles FROM Job WHERE JobBytes <> 0 AND JobStatus = 'T' AND PurgedFiles = 0;" | grep -v "Restore" | grep "Job" | grep $lines | awk '{ print $2 }'`
